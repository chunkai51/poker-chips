...
- 可打开“席位与身份管理”，处理玩家重连、身份接管、管理员授权，以及结算后的座次/筹码/离桌调整
- 筹码归零的玩家会标记为待补码，下一手自动跳过
- 初步支持边池计算和结算
- 支持结算后开始下一局并轮转 Dealer
- 多人房间使用玩家码和管理码：玩家可换设备重新接管身份，管理员权限可授予玩家
- 开局前可选择“本地模式”或“多人房间”，多人房间支持创建/加入房间和随时绑定玩家座位
- 多人房间权限分层：管理员管理准备页和牌桌；已绑定玩家只能由本人操作，未绑定玩家由管理员代管；发牌确认由 Dealer 或其代管者完成
- 结算预览和开始下一局需要所有相关玩家确认；未绑定玩家的确认由管理员代管，任一设备仍可取消结算预览并回到赢家选择
...
3. 多人房间中可以点击“创建房间”，或输入房间 ID 后点击“加入房间”。创建房间会生成管理码，请保存。
4. 设置初始筹码和大盲注。
5. 管理员添加至少两位玩家。多人房间中，每台设备可以在“席位与身份管理”里绑定或重新接管自己的座位；首次绑定会生成玩家码。
6. 管理员点击“开始游戏”。
...
12. 游戏中如需换设备，打开“席位与身份管理”输入玩家码接管；结算后如需补码、回桌、删除玩家或调整座次，由管理员在同一窗口处理。
13. 房间模式下所有下一局可参与玩家确认后开始下一局；本地模式可直接开始。

多人使用时，请让所有设备进入同一个房间 ID。页面会通过 Firebase 同步玩家、奖池、行动轮次和日志。

当前权限仍是前端体验层：房间会记录管理码哈希、管理员玩家、成员会话和玩家归属，界面和写入入口会按身份限制操作；如果要抵御恶意客户端，还需要 Firebase Auth 和 Realtime Database Security Rules。
...
- `src/identity.js`: 本机客户端 ID、房间模式、成员列表、管理码/玩家码和玩家归属字段的兼容身份层。
...
- 权限层目前属于前端体验级限制，已按管理员、玩家本人、Dealer 和全员确认控制主要写入入口；安全级限制需要配套 Firebase Auth 和数据库规则。
...
- 接入 Firebase Auth 和数据库规则，强化房间权限安全边界
- 增加导出牌局日志或恢复历史牌局能力
- 优化小屏幕上的密集操作体验
