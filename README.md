...
4. 设置初始筹码和大盲注。
5. 添加至少两位玩家。多人房间中，每台设备可以点击玩家旁边的“认领”来绑定自己的座位。
6. 点击“开始游戏”。
...
11. 所有设备都会看到结算预览；确认后结算，取消则回到赢家选择。
12. 结算后如需补码、回桌或调整座次，点击“牌桌管理”。
13. 点击“开始下一局”继续游戏。

多人使用时，请让所有设备进入同一个房间 ID。页面会通过 Firebase 同步玩家、奖池、行动轮次和日志。

当前身份绑定仍是兼容模式：认领座位会影响本机视角和后续权限设计，但现在还不会阻止其他设备点击公共操作。严格的“只能操作自己的玩家”等权限会在后续阶段开启。
...
- 身份层目前保持兼容模式：房间会记录房主、成员和玩家归属字段，UI 已支持认领座位和本机视角旋转；但未绑定玩家仍按旧的公共操作方式运行，“只能操作自己的玩家”“所有玩家确认下一局”等权限会在后续阶段实现。
