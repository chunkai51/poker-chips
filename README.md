...
- 开局前可选择“本地模式”或“多人房间”，多人房间支持创建/加入房间和认领玩家座位
- 多人房间权限分层：房主管理准备页和牌桌管理；已绑定玩家只能由本人操作，未绑定玩家由房主代管；发牌确认由 Dealer 或其代管者完成
- 结算预览和开始下一局需要所有相关玩家确认；未绑定玩家的确认由房主代管，任一设备仍可取消结算预览并回到赢家选择
- 多人房间中，已认领的玩家默认在本设备视角下位于牌桌下方中间；也可手动旋转本机牌桌视角，点击“以我为底”可恢复，旋转不会同步给其他设备
- 响应式界面，适配桌面和手机浏览器
...
3. 多人房间中可以点击“创建房间”，或输入房间 ID 后点击“加入房间”。
4. 设置初始筹码和大盲注。
5. 房主添加至少两位玩家。多人房间中，每台设备可以点击玩家旁边的“认领”来绑定自己的座位。
6. 房主点击“开始游戏”。
...
12. 结算后如需补码、回桌或调整座次，房主点击“牌桌管理”。
13. 房间模式下所有下一局可参与玩家确认后开始下一局；本地模式可直接开始。

多人使用时，请让所有设备进入同一个房间 ID。页面会通过 Firebase 同步玩家、奖池、行动轮次和日志。

当前权限仍是前端体验层：房间会记录房主、成员和玩家归属，界面和写入入口会按身份限制操作；如果要抵御恶意客户端，还需要 Firebase Auth 和 Realtime Database Security Rules。
...
- 权限层目前属于前端体验级限制，已按房主、玩家本人、Dealer 和全员确认控制主要写入入口；安全级限制需要配套 Firebase Auth 和数据库规则。
